...
void ProceessBonusBannerAndScoring(void)
{
    ...
    DmaCopy16(3, gBoardConfig.fieldLayout.objPaletteData[0] + var0 * 0x20, (void *)0x05000300, 0x20);

    if (group->available)
    {
        group->baseX = 120;
        group->baseY = gCurrentPinballGame->bonusScrollOffsetY + 50;
        for (i = 0; i < 3; i++)
        {
...
void UpdatePointerSpritePosition(void)
{
    s16 i;
    struct SpriteGroup *group;
    struct OamDataSimple *oamSimple;
...
void TransitionBackToMainBoard(void)
{
    u8 temp;
