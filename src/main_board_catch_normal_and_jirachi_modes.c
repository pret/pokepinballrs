...
void UpdateCatchEmMode(void)
{
    ...
    case CATCH_EM_SUBSTATE_SETUP_TILE_OVERLAY:
        ...
        gCurrentPinballGame->boardSubState++;
        for (i = 0; i < 6; i++)
            gCurrentPinballGame->catchTilePalette[i] = PAL_IX_15;
        break;
        ...
        if (gMain.modeChangeFlags == MODE_CHANGE_NONE)
        {
            if (gMain.selectedField == FIELD_RUBY)
            {
                ...
                else if (gCurrentPinballGame->modeAnimTimer == 93)
                {
                    ...
                    DmaCopy16(3, gModeBanner_Pals[2], OBJ_PLTT_SLOT(PAL_IX_14), PLTT_SLOT_SIZE);
    ...
    case CATCH_EM_SUBSTATE_LOAD_MON_SPRITE:
        ...
        else
        {
            ...
            DmaCopy16(3, gCatchMonAppearFx_Pal, OBJ_PLTT_SLOT(PAL_IX_14), PLTT_SLOT_SIZE);
    ...
    case CATCH_EM_SUBSTATE_SETUP_CATCH_HIT_COUNT:
        ...
        DmaCopy16(3, gCaptureHit_Pal, OBJ_PLTT_SLOT(PAL_IX_15), PLTT_SLOT_SIZE);
        DmaCopy16(3, gCatchSpritePalettes, OBJ_PLTT_SLOT(PAL_IX_13), PLTT_SLOT_SIZE);
        ...
        if (gMain.modeChangeFlags == MODE_CHANGE_NONE)
        {
            if (gMain.selectedField == FIELD_RUBY)
            {
                ...
                else if (gCurrentPinballGame->modeAnimTimer == 93)
                {
                    ...
                    DmaCopy16(3, gModeBanner_Pals[5], OBJ_PLTT_SLOT(PAL_IX_14), PLTT_SLOT_SIZE);
                    gMain.blendControl = 0xCE;
                }
                else if (gCurrentPinballGame->modeAnimTimer == 73)
                {
