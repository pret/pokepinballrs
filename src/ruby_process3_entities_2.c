...
void UpdateNuzleafEntity(void)
{
    ...
    group = &gMain.spriteGroups[SG_RUBY_71];
    var0 = 0;
    var1 = 0;
...
void AnimateRubyShopDoor(void)
{
    ...
    group = &gMain.spriteGroups[SG_RUBY_65];
    ...
    for (i = 0; i < 2; i++)
    {
        oamSimple = &gMain.spriteGroups[SG_RUBY_65].oam[i];
...
void RubyPondTriBumperHandleHitAndDraw(void)
{
    ...
    group = &gMain.spriteGroups[SG_RUBY_62];
    if (gCurrentPinballGame->rubyPondState == RUBY_POND_STATE_LOTAD)
    {
        for (i = 0; i < 3; i++)
        {
...
void AnimateSharpedoEntity(void)
{
    ...
    group = &gMain.spriteGroups[SG_RUBY_61];
