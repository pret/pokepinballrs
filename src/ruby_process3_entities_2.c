...
void DrawWhiscash(void)
{
    ...
    {
        frameIx = frameData[0];
        DmaCopy16(3, gWhiscash_Pals[3*gCurrentPinballGame->activePaletteIndex], OBJ_PLTT_SLOT(PAL_IX_9), PLTT_SLOT_SIZE);
...
void RubyPondTriBumperHandleHitAndDraw(void)
{
                ...
                if (gCurrentPinballGame->catchTilesBumperAcknowledgedCount == 6)
                {
                    if (gCurrentPinballGame->catchTilesBoardAcknowledged == 0)
                    {
                        if (gCurrentPinballGame->rubyPondState == RUBY_POND_STATE_LOTAD)
                        {
                            ...
                            DmaCopy16(3, gModeBanner_Pals[6], OBJ_PLTT_SLOT(PAL_IX_14), PLTT_SLOT_SIZE);
                        }
                        else
                        {
                            gMain.modeChangeFlags |= MODE_CHANGE_BANNER;
                            ...
                            DmaCopy16(3, gModeBanner_Pals[1], OBJ_PLTT_SLOT(PAL_IX_14), PLTT_SLOT_SIZE);
                        }
    ...
    if (gCurrentPinballGame->rubyPondState == RUBY_POND_STATE_LOTAD)
    {
        ...
        DmaCopy16(3, gLotadBumper_Pals[3*gCurrentPinballGame->activePaletteIndex], OBJ_PLTT_SLOT(PAL_IX_9), PLTT_SLOT_SIZE);
    }
    else // chinchou
    {
        for (i = 0; i < 3; i++)
        ...
        DmaCopy16(3, gChinchouBumper_Pals[3*gCurrentPinballGame->activePaletteIndex], OBJ_PLTT_SLOT(PAL_IX_9), PLTT_SLOT_SIZE);
    }
