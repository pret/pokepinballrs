...
def parse_params_line(line: str) -> tuple[List[str], Dict[str, Any]]:
    """
    Accepts shorthand:
      -o  -> -oam
      -w  -> -width
    ...
    needs_value = {"-width", "-mheight", "-mwidth"}
...
def reset_json_barebones(existing: Optional[Dict[str, Any]], base_name: str) -> Dict[str, Any]:
    """
    "Wipe it down to bare bones format (up through the start of the segments array.)"
    We interpret that as:
      - preserve any top-level keys OTHER than "files" if a JSON already exists (metadata/defaults/etc),
      - replace "files" with just one entry for this base and an empty segments array.
    """
    root: Dict[str, Any] = {}
    ...
    root["files"] = [{"gfx_filename": base_name, "segments": []}]
    return root
...
def main() -> None:
    ...
    print("Parameter Shorthand: -o (-oam), -w (-width), -mh (-mheight), -mw (-mwidth)")
    ...
    while True:
        ...
        try:
            gbagfx_args, json_fields = parse_params_line(param_line)
        ...
        if not ok_gfx:
            # Clean up intermediate .4bpp if it exists
        ...
        # SUCCESS — delete intermediate .4bpp
        if out_4bpp_unique.exists():
            try:
                out_4bpp_unique.unlink()
            except Exception as e:
        ...
        if not ok:
            print("Retrying with the same unique (outputs will be overwritten).\n")
            continue

        # Step 4/5 - prompt name entry/rename
        while True:
            ...
            if out_png_named.exists():
                ...
                if not overwrite:
                    print("Okay—pick a different name.\n")
                    continue  # ← now this only re-prompts for name
